...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        qval = 0
        nextstages  = self.mdp.getTransitionStatesAndProbs(state, action) 

        for nstage in nextstages:
            reward = self.mdp.getReward(state, action, nstage)
            qval += nstage[1] * (reward + (self.discount * self.values[nstage[0]])) 
